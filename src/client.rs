...
impl LinkedInClient {
    ...
    pub async fn login(email: &str, password: &str) -> Result<Self> {
        ...
        if document.select(&email_selector).next().is_none() {
            return Err(
                LinkedInError::ElementNotFound(
                    "Email input field not found. LinkedIn may have changed their login page structure.".to_string()
                )
            );
        }
        ...
        let csrf_token = document
            ...
            .ok_or_else(||
                LinkedInError::ElementNotFound("CSRF token not found on login page.".to_string())
            )?;
        ...
        if final_url.contains("/checkpoint") && !final_url.contains("/feed") {
            ...
            let error_selector = Selector::parse(
                ".form__label--error, .alert, .msg--error, .error"
            ).unwrap();
            ...
            return Err(
                LinkedInError::Unknown(
                    "Login failed. You may be stuck at a checkpoint. Try logging in manually first.".to_string()
                )
            );
        }
        ...
        if
            ...
            (final_url.contains("/feed") || final_url.contains("/in/"))
        {
            return Ok(client);
        }
        ...
        if
            response_text.contains("global-nav") ||
            response_text.contains("linkedin.com/feed") ||
            response_text.contains("li-header")
        {
            return Ok(client);
        }
